  ...
  render() {
    ...
    return (
      ...
      >
        ...
        <h2 style={{ display: noAccounts ? '' : 'none' }}>
          Please refresh to try again, and ensure you are using a UUID
        ...
        <Card.Group style={{ margin: 'auto', width: '100%' }} centered>
          {this.props.sorted.map((item, i) => {
            ...
            if (
              ...
              header = (
                <a
                  className="card-content-light"
                  href={`https://rpm.newrelic.com/accounts/${item.instances[0].accountId}`}
                  ...
                  <Table celled inverted={false} basic="very">
                    ...
                    <Table.Body>
                      <Table.Row>
                        ...
                        <Table.Cell>
                          $
                          {(item.nonOptimizedCost * 720)
                            .toFixed(2)
                            .replace(/\d(?=(\d{3})+\.)/g, '$&,')}
                        </Table.Cell>
                        <Table.Cell>
                          $
                          {(item.nonOptimizedCost * 720 * 12)
                            .toFixed(2)
                            .replace(/\d(?=(\d{3})+\.)/g, '$&,')}
                        </Table.Cell>
                      </Table.Row>
                      <Table.Row>
                        ...
                        <Table.Cell>
                          $
                          {(item.optimizedCost * 720)
                            .toFixed(2)
                            .replace(/\d(?=(\d{3})+\.)/g, '$&,')}
                        </Table.Cell>
                        <Table.Cell>
                          $
                          {(item.optimizedCost * 720 * 12)
                            .toFixed(2)
                            .replace(/\d(?=(\d{3})+\.)/g, '$&,')}
                        </Table.Cell>
                      </Table.Row>
                    ...
                    <Table.Footer>
                      <Table.Row>
                        <Table.HeaderCell
                          ...
                          colSpan={3}
                        >
                          {item.nonOptimizedCount} of {item.totalInstances}{' '}
                          Instances to Optimize |{' '}
                          {item.nonOptimizedCost > 0
                            ? (
                                (Math.abs(item.saving) /
                                  item.nonOptimizedCost) *
                                100
                              ).toFixed(2)
                            : 0}
                          % Cost Saving
