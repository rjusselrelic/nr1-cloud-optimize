...
export default class CloudOptimizeApm extends React.Component {
  ...
  async fetchData() {
    ...
    const entityData = ((result || {}).actor || {}).entity || null;
    ...
    if (entityData) {
      ...
      if (hosts && hosts.length > 0) {
        ...
        const {
          ...
          hostMetrics,
          hasCloud,
        } = this.calculateHostMetricsAndInstanceData({
          systemSamples,
          uniqueDailyHosts,
        });
  ...
  calculateHostMetricsAndInstanceData({ systemSamples, uniqueDailyHosts }) {
    ...
    const { entityData, networkSamples } = this.state;
